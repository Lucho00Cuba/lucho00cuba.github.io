...
const sort = (posts: Post[]) => posts.sort(({data:a}, {data:b}) => {
  if(a.top && b.top) {
    // If 'top' values are the same, the latest article comes first
    if(a.top == b.top) return b.publishDate!.valueOf() - a.publishDate!.valueOf();
    // If 'top' values are different, the one with a larger 'top' comes first
    else return b.top - a.top; 
    // If only one article has 'top' defined, the one with 'top' comes first
  } else if(a.top && !b.top) {
    return -1;
  } else if(!a.top && b.top) {
    return 1;
  } else {
    // If neither has 'top', the latest article comes first
